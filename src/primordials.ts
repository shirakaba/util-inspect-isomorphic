...
//
// The fork is mostly just to remove the hard dependency on eval(), which is not
// implemented by engines such as Hermes:
...
/* c8 ignore start */
const getValue = <T extends {}, K extends keyof T>(obj: T, prop: K): T[K] => {
...
const setTimeout = Object.freeze(
  Object.assign(
    (...args: Parameters<typeof setTimeout_>): ReturnType<typeof setTimeout_> =>
      setTimeout_(...args),
    { ...setTimeout_ },
  ),
);
...
const primordials = OBJECT.defineProperties(
  ...
  }),
  {
    ...
    ErrorStackTraceLimit: {
      enumerable: true,
      get: () =>
        SafeReflect.getOwnPropertyDescriptor(Error, "stackTraceLimit")?.value,
      set: (value: number) => {
